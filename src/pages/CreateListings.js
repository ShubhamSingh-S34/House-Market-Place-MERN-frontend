...
function CreateListings() {
  ...
  const onSubmit = async (e) => {
    ...
    const storeImage = async (image) => {
      return new Promise((resolve, reject) => {
        ...
        uploadTask.on(
          ...
          (error) => {
            console.log('ERROR IN storeImages function : ', error);
            reject(error)
          },
          () => {
    ...
    try {
      const response = await axios.post('https://house-market-place-backend.onrender.com/api/listings', formDataCopy);
      console.log(response.data);
      const listingId = response.data.data._id;
      console.log(listingId);
      setLoading(false);
      toastifySuccess("Listing saved");
      console.log(`/category/${formDataCopy.type}/${listingId}`);
      navigate(`/category/${formDataCopy.type}/${listingId}`);
    }
    catch (e) {
      console.log('ERROR IN THE REQUEST, /api/listings ----> ', e);
    }
